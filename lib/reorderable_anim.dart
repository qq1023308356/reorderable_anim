...
import 'reorderable_anim_notification.dart';
import 'reorderable_anim_widget.dart';

typedef DragItems<T> = Widget Function({required T data, required Widget child, required Key key});

class ReorderableAnim<T extends Object> extends StatefulWidget {
  const ReorderableAnim({
  ...
  @override
  State<StatefulWidget> createState() => ReorderableAnimState<T>();
}

class ReorderableAnimState<T extends Object> extends State<ReorderableAnim<T>> {
  Timer? _timer;
  ...
  Widget build(BuildContext context) {
    return ReorderableAnimNotification(
  ...
  bool setWillAccept(DragTargetDetails<T> details, T data) {
    ...
    if (status == AnimationStatus.completed) {
      ...
      _timer = Timer(const Duration(milliseconds: 200), () {
        if (!ReorderableAnimNotification.isScroll) {
    ...
    return ReorderableAnimWidget(
      ...
      child: DragTarget<T>(
        onWillAcceptWithDetails: (DragTargetDetails<T> details) {
          if (isDragStart && !ReorderableAnimNotification.isScroll) {
